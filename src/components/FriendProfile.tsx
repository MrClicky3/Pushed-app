        ...
        <div>
          ...
          {loading ? (
            ...
            <div className="space-y-1.5">
              <StatCard label="Volume" value={`${Math.round(toDisplay(w.volume)).toLocaleString()} ${unit}`} sub={WINDOWS.find(x => x.key === win)!.label} />
              <StatCard label="Sets logged" value={w.sets.toLocaleString()} sub={WINDOWS.find(x => x.key === win)!.label} />
              <StatCard label="Active days" value={String(w.active_days)} sub={WINDOWS.find(x => x.key === win)!.label} />
            </div>
          ) : (
